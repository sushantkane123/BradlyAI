"""
CyCraft AI - Driverless SOC & Automated Incident Response Application Entrypoint
"""
...
from cycraft.database import engine, Base, SessionLocal
...
def seed_database():
    ...
    try:
        if db.query(AlertModel).count() == 0:
            print("🌱 Seeding default CyCraft security alerts to SQLite database...")
            default_alerts = [
                {
                    "id": "ALT-8921",
                    "severity": "CRITICAL",
                    "title": "Suspicious Powershell Execution & Registry Modification",
                    "endpoint": "DEV-WIN-SRV09",
                    "ip": "192.168.10.45",
                    ...
                    "storyline": [
                        ...
                        {"time": "14:02:19", "event": "CyCraft AI detected behavior, killed process, and isolated network interface"}
                    ]
                },
                {
                    "id": "ALT-8920",
                    "severity": "HIGH",
                    "title": "Unauthorized Lateral Movement via SMB Exploitation",
                    "endpoint": "FIN-WRK-102",
                    "ip": "192.168.20.12",
                    ...
                    "storyline": [
                        ...
                        {"time": "13:50:06", "event": "CyCraft AIR triggered local firewall lockdown and revoked active Kerberos tickets"}
                    ]
                },
                {
                    "id": "ALT-8919",
                    "severity": "HIGH",
                    "title": "Exfiltration Attempt to Known Tor Exit Node",
                    "endpoint": "ENG-MAC-404",
                    "ip": "192.168.15.88",
                    ...
                    "storyline": [
                        ...
                        {"time": "13:33:00", "event": "CyCraft AIR blocked external IP at gateway and flagged for analyst review"}
                    ]
                },
                {
                    "id": "ALT-8918",
                    "severity": "MEDIUM",
                    "title": "Anomalous Cloud IAM Policy Privilege Escalation",
                    "endpoint": "AWS-IAM-US-EAST",
                    "ip": "54.210.85.12",
                    ...
                    "storyline": [
                        ...
                        {"time": "12:55:22", "event": "CyCraft Cloud AI intercepted API call via AWS EventBridge and rolled back IAM policy"}
                    ]
                }
            ]
            ...
            for alt in default_alerts:
                db_alt = AlertModel(
                    id=alt["id"], severity=alt["severity"], title=alt["title"],
        ...
        if db.query(AssetModel).count() == 0:
            print("🌱 Seeding default ASM assets to SQLite database...")
            default_assets = [
                {
                    "name": "core-auth-api.cycraft.internal", "type": "Web Service", "ip": "192.168.1.10",
                ...
                {
                    "name": "vpn-gateway-apac.cycraft.com", "type": "Network Gateway", "ip": "203.0.113.45",
            ...
            for ast in default_assets:
                db_ast = AssetModel(
                    name=ast["name"], type=ast["type"], ip=ast["ip"], owner=ast["owner"],
...
app = FastAPI(
    ...
    description="CyCraft AI multi-model machine learning mesh backend API providing automated incident response and a true Driverless SOC experience.",
    docs_url="/docs",
    redoc_url="/redoc"
)

# Manage Application Lifecycle (Start / Stop Live Background Simulation Worker)
@app.on_event("startup")
def start_app_events():
...
# Custom Middleware for Execution Timing & Security Branding
@app.middleware("http")
async def add_process_time_header(request: Request, call_next):
    ...
    response.headers["X-Autonomous-SOC"] = "CyCraft Multi-Model Active Engine"
    return response
...
async def custom_http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(
        ...
        content={
            "error_code": "CYCRAFT_API_EXCEPTION",
            "message": exc.detail,
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    return JSONResponse(
        ...
        content={
            "error_code": "CYCRAFT_VALIDATION_ERROR",
